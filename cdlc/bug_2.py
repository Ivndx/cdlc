...
class BugAlgorithmClass(Node):
    def __init__(self):
        ...
        self.get_logger().info("Reactive navigation node initiated !!")
    ...
    def follow_wall(self, direction):
        ...
        if direction == 'left':
            t2, r2 = math.radians(45), self.robot_view.get('front_left')
            t1, r1 = math.radians(90), self.robot_view.get('left')
        else:
            t2, r2 = math.radians(-45), self.robot_view.get('front_right')
            t1, r1 = math.radians(-90), self.robot_view.get('right')
    ...
    def state_machine(self):
        if len(self.current_pose) > 0:
            ...
            if self.state == "go_to_goal" and self.atTarget():
                self.state = "stop_robot"
            if self.state == "go_to_goal" and self.isObstacleTooClose():
                self.state = "follow_wall"
            if self.state == "follow_wall" and self.isObstacleCleared():
                self.state = "go_to_goal"
